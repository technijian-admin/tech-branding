// Gossamer Space Frames (GSF) — Executive Summary (forwardable 3-4pp hook artifact)
// Distilled from the full blueprint. FACTS-ONLY, market figures attributed, 2012 timeline stated plainly.
// Engineer-to-engineer, for-profit voice; the reframe: NOT managed IT/security, but AI to get found & win work.
...
const children = [
  ...
  centered('Gossamer Space Frames', { size: 40, color: DARK_CHARCOAL, bold: true, after: 80 }),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 60 }, children: [new TextRun({ text: 'AI Growth Blueprint — get found, win more work, in a solar market that is growing again', size: 21, italics: true, color: BRAND_GREY, font: FONT_BODY })] }),
  new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 170 }, children: [new TextRun({ text: 'Prepared for Glenn Reynolds, Gossamer Space Frames  ·  ' + TODAY + '  ·  Confidential', size: 17, color: BRAND_GREY, font: FONT_BODY })] }),
  kpiRow([
  ...
  calloutBox('First — this is not a managed-IT or security pitch',
    'When we first reached out we led with managed IT and a security assessment, and you said you are not there yet. Fair enough — this is something different and, for a firm like yours, more valuable: using AI to get found and win more work in a concentrated-solar market that, after a quiet decade, is growing again. That is commercial growth, not IT.', CORE_ORANGE),
  p('Gossamer Space Frames designs and fabricates space-frame structures for concentrated solar power and architecture. Founded in 1999 by Glenn Reynolds and partners, it pioneered the LAT 73 Large Aperture Trough with 3M — a 7.3-meter-aperture collector, NREL-verified at better than 99% optical intercept, that cut solar-field cost by more than 25% — and helped build the Project-of-the-Year Martin plant for Florida Power & Light. The engineering is world-class. This summary states only verified facts, attributes all market figures, and defers company-specific numbers to discovery.', { spaceBefore: 120, spaceAfter: 120 }),
  pRuns([{ text: 'The problem is not the engineering — it is that almost no one buying these structures today can find you. ', bold: true, color: DARK_CHARCOAL }, { text: 'Gossamer’s public presence largely froze around 2012, while the parabolic-trough market quietly returned. The plan has two parts: ' }, { text: 'be found ', bold: true, color: DARK_CHARCOAL }, { text: '— a modern technical presence that turns the LAT, the NREL data, and the patents into search-and-AI-citable proof; and ' }, { text: 'win more work ', bold: true, color: DARK_CHARCOAL }, { text: '— AI that watches the project pipeline, finds the developers and EPCs sourcing now, and drafts the technical outreach and proposals. AI works the front end and drafts; the team verifies every claim and owns every relationship — and it never touches the engineering.' }], { spaceAfter: 60 }),

  subHeader('What we verified'),
  bulletRuns([{ text: 'World-class, proven IP: ', bold: true, color: DARK_CHARCOAL }, { text: 'the LAT 73 (7.3 m aperture, >100x concentration, NREL-verified >99% intercept, >25% field-cost reduction), built with 3M; multiple U.S. patents naming Glenn Reynolds, PE; and the Project-of-the-Year Martin plant for FPL.' }]),
  bulletRuns([{ text: 'A market that has come back: ', bold: true, color: DARK_CHARCOAL }, { text: 'analysts put parabolic-trough CSP back on a growth track, with the fastest demand in the Middle East and North Africa and a fast-growing industrial-process-heat adjacency — trough remains the dominant CSP technology.' }]),
  bulletRuns([{ text: 'The gap is reach, not proof: ', bold: true, color: DARK_CHARCOAL }, { text: 'a 2012-era website, a barely-followed LinkedIn page, no living case studies, and near-zero visibility for the terms a developer actually searches — while more-visible competitors take the calls.' }]),
  bulletRuns([{ text: 'Honest about timing: ', bold: true, color: DARK_CHARCOAL }, { text: 'the LAT / 3M debut was 2011–2012, not recent; we treat it as a proven platform whose moment has returned, and we cite no Gossamer revenue figure because none is public and verified.' }]),
  calloutBox('A correction we owe you, up front', 'Our first note called the LAT / 3M debut "recent." On research, it was proven out in 2011–2012, independently verified by NREL. We would rather be precise than flattering — and that precision is the standard behind every figure here.', TEAL),

  subHeader('The engine'),
  p('Three columns — be found and tell the story, win more work, and respond and operate faster — each delivered by a specific Technijian service, with AI kept entirely on the commercial front end.', { spaceAfter: 60 }),
  diagramImage(diagBuf('architecture.png'), 'AI growth engine', 600),

  subHeader('Investment & the path forward'),
  pRuns([{ text: 'Priced from real ranges, no invented numbers. ', bold: true, color: DARK_CHARCOAL }, { text: 'We price from published My SEO / My AI / My Dev ranges, start small — get found first, because it is the cheapest, fastest win — and model returns only after discovery. For a project-based firm, return is value at stake: a single additional contract is worth far more than a year of this engine, so it pays for itself if it wins even one opportunity that would never have reached you.' }], { spaceAfter: 100 }),
  p('The easiest first step is a short working session on where AI creates measurable value for Gossamer — no obligation, and nothing about managed IT. The full blueprint ends with a structured questionnaire, "Questions to Complete the Analysis," that turns this summary into a costed, calibrated plan scoped to a small firm.', { spaceAfter: 110 }),
...
const docHeader = new Header({ children: [
  new Table({ width: { size: CONTENT_W, type: WidthType.DXA }, columnWidths: [Math.floor(CONTENT_W * 0.5), CONTENT_W - Math.floor(CONTENT_W * 0.5)], borders: { ...noBorders, bottom: { style: BorderStyle.SINGLE, size: 6, color: CORE_BLUE } }, rows: [new TableRow({ children: [
    ...
    new TableCell({ width: { size: CONTENT_W - Math.floor(CONTENT_W * 0.5), type: WidthType.DXA }, borders: noBorders, verticalAlign: VerticalAlign.CENTER, children: [new Paragraph({ alignment: AlignmentType.RIGHT, children: [new TextRun({ text: 'Gossamer Space Frames  ·  Executive Summary', size: 16, color: BRAND_GREY, font: FONT_BODY })] })] }),
  ] })] }),
  new Paragraph({ children: [new TextRun({ text: '' })] }),
] });
...
const doc = new Document({
  creator: 'Technijian', title: 'Gossamer Space Frames — Executive Summary', description: 'Forwardable executive summary for Gossamer Space Frames, prepared by Technijian.',
...
const outPath = path.join(__dirname, 'Gossamer-Space-Frames-Executive-Summary.docx');
