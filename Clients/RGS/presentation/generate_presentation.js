...
function whoSlide() {
  ...
  const cards = [
    ...
    { label: "Security-first", title: "Built in, not bolted on", band: COLORS.teal, body: "CISSP-led, delivering 24/7 managed detection & response — our My Jian SIEM correlating across CrowdStrike/Huntress MDR, with our analysts triaging. Security isn't an add-on line item for us — it's how we run everything." },
...
function securitySlide() {
  ...
  const layers = [
    ...
    { title: "24/7 MDR & response", band: COLORS.dark, body: "Eyes-on-glass monitoring with a defined response when something looks wrong — the around-the-clock coverage a small team can't staff alone." },
  ];
  layers.forEach((l, i) => {
