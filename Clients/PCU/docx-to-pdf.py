"""
DOCX -> PDF for the PCU (Pacific Utility Installation) deliverable, via Word COM (pywin32).
Updates the Table of Contents fields before exporting. Pass an ABSOLUTE path.
Usage: python Clients/PCU/docx-to-pdf.py [absolute-docx-path]
"""
import os
import sys
import time
...
DOCX = os.path.join(HERE, 'Pacific-Utility-AI-Growth-Report.docx')
WD_FORMAT_PDF = 17
...
def convert(docx):
    ...
    if not os.path.exists(docx):
        ...
        return 1
    # Clear any stale/locked PDF so SaveAs doesn't wedge on a lock.
    if os.path.exists(pdf):
        try:
            os.remove(pdf)
        except Exception as e:
            print(f'  could not remove stale pdf: {e}')
