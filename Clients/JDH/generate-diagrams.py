"""
generate-diagrams.py - JDH  JDH Pacific (Fullerton CA contract manufacturer / supply-chain partner)
...
Output:  Clients/JDH/diagrams/model.png         (Figure 02.0 - How a contract manufacturer wins)
         Clients/JDH/diagrams/personas.png      (Figure 05.0 - OEM-buyer matrix)
         Clients/JDH/diagrams/competitive.png   (Figure 06.0 - Supply-chain capability x AI maturity)
         Clients/JDH/diagrams/architecture.png  (Figure 10.0 - AI growth + integration engine)
         Clients/JDH/diagrams/timeline.png      (Figure 12.0 - 90/180/270 roadmap)
Usage:   python Clients/JDH/generate-diagrams.py
"""
...
# ============================================================================
# DIAGRAM 1 - HOW A CONTRACT MANUFACTURER WINS (900 x 540)
# OEM demand on top, the parts-program funnel in the middle, AI band below
# ============================================================================

def build_model_svg():
    title = f"""
  <text x="40" y="34" font-family="Segoe UI,Arial" font-size="18" font-weight="700" fill="{DARK}">How a Contract Manufacturer &amp; Supply Partner Wins - Specify, Quote, Qualify, Deliver, Hold</text>
  <text x="40" y="55" font-family="Segoe UI,Arial" font-size="12" fill="{GREY}">A named universe of industrial OEM accounts on top; JDH's multi-origin supply chain underneath; AI on the quote and qualification path.</text>"""

    # Demand sources (4 boxes)
    DX = [20, 240, 460, 680]
    DW = 198
    demand = (
        box(DX[0], 74, DW, 46, OFF, LIGHT, "AI-SEARCH / AEO",     "sourcing + NPI engineers",   DARK, GREY, 13)
        + box(DX[1], 74, DW, 46, OFF, LIGHT, "SUPPLIER-DIVERSITY", "MBE Tier-2 programs",        DARK, GREY, 13)
        + box(DX[2], 74, DW, 46, OFF, LIGHT, "REFERRAL / REPUTATION","35-yr OEM track record",   DARK, GREY, 13)
        + box(DX[3], 74, DW, 46, OFF, LIGHT, "ABM OUTBOUND",       "trigger-based outreach",     DARK, GREY, 13)
    )
    label = f"""<text x="450" y="140" font-family="Segoe UI,Arial" font-size="11" font-weight="700" fill="{GREY}" text-anchor="middle">DEMAND SIDE - OEM SOURCING / NPI / SUPPLIER-DIVERSITY BUYERS</text>"""
    conns = ""
    for dx in DX:
        cx = dx + DW // 2
        conns += f"""<line x1="{cx}" y1="120" x2="450" y2="150" stroke="#ADB5BD" stroke-width="1.2"/>"""

    # Parts-program funnel stages
    ...
    stages = (
        ...
        + box(SX[2], SY, SW, SH, BLUE,   BLUE,   "QUALIFY",       "PPAP / FAI",          WHITE, "#C9E3F4", 12)
        + box(SX[3], SY, SW, SH, ORANGE, ORANGE, "AWARD",         "PO + agreement",      DARK,  DARK,      12)
        + box(SX[4], SY, SW, SH, BLUE,   BLUE,   "DELIVER",       "COA + scorecard",     WHITE, "#C9E3F4", 12)
  ...
  <text x="44" y="{BY+28}" font-family="Segoe UI,Arial" font-size="13" font-weight="700" fill="{DARK}">Where Technijian AI plugs in - on your quote, qualification, and tariff workflow, not a black-box pricing engine</text>
  <text x="44" y="{BY+58}" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{BLUE}">Get Found &amp; Specified</text>
  <text x="44" y="{BY+76}" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}">Multi-Agent SEO + AEO - authority content on tariff-resilient sourcing, MBE Tier-2 value, and total landed cost</text>
  <text x="44" y="{BY+104}" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{ORANGE}">Win the Quote &amp; Qualification Race - the core build</text>
  <text x="44" y="{BY+122}" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}">AI RFQ/drawing intelligence - quote drafting + should-cost - PPAP/FAI/COA assembly - named-account intelligence</text>
  <text x="44" y="{BY+150}" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{TEAL}">Hold &amp; Grow</text>
  <text x="44" y="{BY+168}" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}">35-yr quote/part/tooling memory - tariff &amp; landed-cost monitor - supplier-scorecard automation - capacity recovery</text>
  <text x="44" y="{BY+196}" font-family="Segoe UI,Arial" font-size="11" font-style="italic" fill="{GREY}">AI drafts the quote, classifies the part, and assembles the cert package. A JDH engineer signs the material cert</text>
  <text x="44" y="{BY+212}" font-family="Segoe UI,Arial" font-size="11" font-style="italic" fill="{GREY}">and a licensed broker signs the customs entry. AI never fabricates an inspection result or a country-of-origin claim.</text>"""
...
# ============================================================================
# DIAGRAM 2 - OEM-BUYER MATRIX (900 x 600)
...
def build_personas_svg():
    ...
    P = [
        ("SOURCE",  "OEM Strategic-Sourcing / Commodity Mgr", 80, 88, BLUE,   WHITE, 32, 0,  54, "middle"),
        ("NPI",     "OEM NPI / Design Engineer",                  46, 78, TEAL,   DARK,  26, 0,   -44, "middle"),
        ("DIVERSE", "OEM Supplier-Diversity / Procurement Lead",  32, 64, GOLD,   DARK,  24, -32, 0, "end"),
        ("MIDMKT",  "Mid-Market OEM Owner / Buyer",               60, 38, ORANGE, DARK,  22, 0,   40, "middle"),
        ("DIST",    "Distributor / Channel Buyer",                86, 26, GREY,   WHITE, 18, 0,   38, "middle"),
    ]
    dots = ""
  ...
  <text x="{X(22)}" y="492" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="#9AA0A6" text-anchor="middle">FLEXIBLE / EMERGING</text>"""
    ...
    legend = f"""
  <circle cx="84"  cy="557" r="7" fill="{BLUE}"/>   <text x="96"  y="562" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">OEM Strategic-Sourcing / Commodity Mgr</text>
  <circle cx="360" cy="557" r="7" fill="{TEAL}"/>   <text x="372" y="562" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">OEM NPI / Design Engineer</text>
  <circle cx="600" cy="557" r="7" fill="{GOLD}"/>   <text x="612" y="562" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">OEM Supplier-Diversity Lead</text>
  <circle cx="84"  cy="581" r="7" fill="{ORANGE}"/> <text x="96"  y="586" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">Mid-Market OEM Owner / Buyer</text>
  <circle cx="360" cy="581" r="7" fill="{GREY}"/>   <text x="372" y="586" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">Distributor / Channel Buyer</text>"""

    return f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="900" height="600" role="img">
  <text x="80" y="28" font-family="Segoe UI,Arial" font-size="18" font-weight="700" fill="{DARK}">The JDH Buyer - Annual Spend x Account Stickiness / Strategic Value</text>
  <text x="80" y="48" font-family="Segoe UI,Arial" font-size="12" fill="{GREY}">The strategic-sourcing manager is the highest LTV; the NPI engineer wins the program early; the diversity lead opens the door.</text>
...
# X = Supply-Chain Capability & Cost Advantage ; Y = Digital & AI Operating Maturity
# ============================================================================
...
def build_competitive_svg():
    ...
    C = [
        ("JDH Today", 76, 24, ORANGE, 28, 36, 6, "start"),
        ("Impro Precision", 88, 44, TEAL, 22, 0, -34, "middle"),
        ("CAB Worldwide", 58, 30, BLUE, 20, -28, 2, "end"),
        ("Bunty LLC", 44, 26, DARK, 16, 26, 2, "start"),
        ("MetalTek", 56, 22, GREY, 16, -26, 2, "end"),
        ("Hitchiner", 50, 18, GREY, 14, 26, 2, "start"),
        ("Signicast / Form", 66, 34, GREY, 16, -26, -2, "end"),
        ("China-direct brokers", 30, 14, GREY, 16, 26, 2, "start"),
    ]
    dots = ""
    for label, s, m, color, r, ldx, ldy, anc in C:
        cx, cy = X(s), Y(m); lx, ly = cx + ldx, cy + ldy
        ring = f"""<circle cx="{cx}" cy="{cy}" r="{r+6}" fill="none" stroke="{ORANGE}" stroke-width="2" stroke-dasharray="4 3"/>""" if label == "JDH Today" else ""
        dots += f"""{ring}
  <circle cx="{cx}" cy="{cy}" r="{r}" fill="{color}"/>
  <text x="{lx}" y="{ly}" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{DARK}" text-anchor="{anc}">{esc(label)}</text>"""

    sx, sy = X(76), Y(24); tx, ty = X(76), Y(82)
    move = f"""
  ...
  <text x="{tx+18}" y="{ty+4}" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{ORANGE}" text-anchor="start">JDH's move</text>"""
  ...
  <text x="{X(24)}" y="492" font-family="Segoe UI,Arial" font-size="11" font-weight="700" fill="#9AA0A6" text-anchor="middle">COMMODITY BROKERS</text>"""
    ...
    return f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="900" height="600" role="img">
  {marker}
  <text x="80" y="28" font-family="Segoe UI,Arial" font-size="18" font-weight="700" fill="{DARK}">Competitive Positioning - Supply-Chain Capability x Digital &amp; AI Operating Maturity</text>
  <text x="80" y="48" font-family="Segoe UI,Arial" font-size="12" fill="{GREY}">Domestic foundries are capable but high-cost; brokers are cheap but bare. The top-right - capable AND AI-enabled - is empty.</text>
  <rect x="80" y="60" width="760" height="440" fill="{OFF}" rx="4"/>
  <line x1="{QVX}" y1="60" x2="{QVX}" y2="500" stroke="{LIGHT}" stroke-width="1.5" stroke-dasharray="5 4"/>
  <line x1="80" y1="{QHY}" x2="840" y2="{QHY}" stroke="{LIGHT}" stroke-width="1.5" stroke-dasharray="5 4"/>
  <line x1="80" y1="60"  x2="80"  y2="500" stroke="#ADB5BD" stroke-width="1.5"/>
  ...
  <text x="460" y="534" font-family="Segoe UI,Arial" font-size="13" font-weight="600" fill="{DARK}" text-anchor="middle">Supply-Chain Capability &amp; Cost Advantage -&gt;</text>
  {quad}{dots}{move}
</svg>"""
...
def build_arch_svg():
  ...
  <text x="148" y="116" font-family="Segoe UI,Arial" font-size="10" fill="#C9E3F4" text-anchor="middle">be the cited supplier answer</text>
  <rect x="316" y="76" width="264" height="48" fill="{ORANGE}" rx="4"/>
  <text x="448" y="100" font-family="Segoe UI,Arial" font-size="13" font-weight="700" fill="{DARK}" text-anchor="middle">WIN THE QUOTE RACE</text>
  <text x="448" y="116" font-family="Segoe UI,Arial" font-size="10" fill="{DARK}" text-anchor="middle">AI-speed quoting + qualification</text>
  ...
  <text x="748" y="116" font-family="Segoe UI,Arial" font-size="10" fill="{DARK}" text-anchor="middle">keep accounts, protect margin</text>"""
    bg = f"""
  ...
  <rect x="616" y="124" width="264" height="412" fill="{OFF}" rx="4"/>"""
    ...
    found = (
        arch_card(20, 136, 256, 88, BLUE, "Answer-Engine (AEO)",
                  "Be cited in Google AI, ChatGPT, and Perplexity for supplier-discovery queries.", "My SEO", BLUE)
        + arch_card(20, 236, 256, 88, BLUE, "Authority Content Engine",
                    "Tariff-resilient sourcing, MBE Tier-2 value, total landed cost.", "My SEO", BLUE)
        + arch_card(20, 336, 256, 88, BLUE, "Named-Account Intelligence",
                    "Trigger monitor - reshoring, new programs, tariff actions, diversity pushes.", "Lead Gen", ORANGE)
        + arch_card(20, 436, 256, 88, BLUE, "Exec LinkedIn Cadence",
                    "Donald + Daniel voice on reshoring and tariff resilience, drafted and approved.", "My SEO", BLUE)
    )
    faster = (
        arch_card(320, 136, 256, 88, ORANGE, "AI RFQ / Drawing Intelligence",
                  "Drawing + spec in - draft quote + should-cost out, 3-model checked.", "My Dev", DARK)
        + arch_card(320, 236, 256, 88, ORANGE, "AI Quote-History Memory",
                    "Instant re-quote of prior parts, tooling, and cost history.", "My Dev", DARK)
        + arch_card(320, 336, 256, 88, ORANGE, "AI PPAP / FAI / COA Engine",
                    "Assemble qualification + cert packages from PC-DMIS data, human-signed.", "My Dev", DARK)
        + arch_card(320, 436, 256, 88, ORANGE, "AI Tariff + Landed-Cost Dashboard",
                    "HTS assist + landed-cost-by-origin model + trade-policy monitor.", "My Dev", DARK)
    )
    keep = (
        arch_card(620, 136, 256, 88, TEAL, "Quote / Part / Tooling Memory",
                  "35 years of quotes and tooling searchable across the team.", "My Dev", DARK)
        + arch_card(620, 236, 256, 88, TEAL, "Supplier-Scorecard Automation",
                    "Keep OEM ratings green - COA on every lot, delivered on time.", "My AI", TEAL)
        + arch_card(620, 336, 256, 88, TEAL, "Tariff-Margin Protection",
                    "Flag quoted jobs whose margin a duty or policy shift erodes.", "My AI", TEAL)
        + arch_card(620, 436, 256, 88, TEAL, "Engineering Capacity Recovery",
                    "Hours back from quoting and docs go to more quotes and better DFM.", "My AI", TEAL)
    )
    arrows = f"""
  ...
  <text x="596" y="324" font-family="Segoe UI,Arial" font-size="10" fill="{GREY}" text-anchor="middle">wins</text>"""
    title = f"""
  <text x="16" y="34" font-family="Segoe UI,Arial" font-size="18" font-weight="700" fill="{DARK}">The AI Growth &amp; Integration Engine - Get Found, Win the Quote Race, Hold &amp; Grow</text>
  <text x="16" y="56" font-family="Segoe UI,Arial" font-size="12" fill="{GREY}">Visibility on the left; the AI quote and qualification engine in the center; account retention and margin protection on the right.</text>"""
    note = f"""
  <text x="16" y="552" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}" font-style="italic">* My SEO - My AI Lead Gen - My AI - My Dev. AI drafts, classifies, assembles, and monitors. A JDH engineer signs the cert; a licensed broker signs the customs entry.</text>"""
...
def build_timeline_svg():
  ...
  <text x="{P2_X+P2_W//2}" y="96" font-family="Segoe UI,Arial" font-size="11" fill="{DARK}" text-anchor="middle">WIN THE QUOTE RACE</text>
  ...
  <text x="820" y="{TL_Y+TL_H+18}" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}" text-anchor="middle">Day 270</text>"""
    ...
    cards = (
        milestone_card(COL_X[0], R1, 232, 92, BLUE, "1", "AEO + Authority Content",
                       "Own AI-search for casting + tariff-resilient sourcing.", "My SEO", BLUE)
        + milestone_card(COL_X[0], R2, 232, 92, BLUE, "2", "Account Intelligence Standup",
                         "Trigger monitor; first 20 named OEM accounts + diversity.", "Lead Gen", ORANGE)
        + milestone_card(COL_X[1], R1, 232, 92, TEAL, "3", "AI RFQ + Quote Engine",
                         "Drawing-to-quote + should-cost; 3-model checked.", "My Dev", DARK)
        + milestone_card(COL_X[1], R2, 232, 92, TEAL, "4", "Quality-Doc + Tariff Dashboard",
                         "PPAP/FAI/COA from PC-DMIS; HTS + landed cost.", "My Dev", DARK)
        + milestone_card(COL_X[2], R1, 236, 92, ORANGE, "5", "Quote Memory + Scorecards",
                         "35-yr quote memory; supplier-scorecard automation.", "My AI", TEAL)
        + milestone_card(COL_X[2], R2, 236, 92, ORANGE, "6", "Roll Across Sites + Productize",
                         "China/India/Mexico inherit the engine; productize.", "My Dev", DARK)
    )
    note = f"""
  <text x="80" y="378" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}" font-style="italic">* Named-account roster, quote volume, and Sage / PC-DMIS integration calibrate after a 30-min discovery call. Estimates shown are conservative.</text>"""
    title = f"""
  ...
  <text x="80" y="48" font-family="Segoe UI,Arial" font-size="12" fill="{GREY}">JDH Pacific + Technijian. Get found and specified, win the quote race, then hold, grow and scale.</text>"""
...
async def main():
    print("Generating JDH diagrams (technijian-diagram skill, SVG)...")
