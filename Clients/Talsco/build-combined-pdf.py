"""
Build a combined PDF: Cover Letter + Resume for Talsco (JVR Sheetmetal & Fabrication, Inc.)
...
def project_table():
    ...
    rows = [
        ["AI Readiness Assessment", "Discovery engagement to map current workflows across quoting, production planning, quality control, inventory management, and shipping; identify top AI opportunities ranked by ROI and feasibility; deliver an executive-ready roadmap"],
        ["Intelligent Quoting", "AI analysis of engineering drawings, specs, and historical job data to produce faster, more accurate quotes \u2014 reducing turnaround time and improving win rates"],
        ["Smart Production Scheduling", "AI-assisted job scheduling, machine utilization optimization, and bottleneck prediction across water jet, laser, CNC, and fabrication operations"],
        ["Quality Intelligence", "AI-powered inspection analytics, defect pattern recognition, SPC trend analysis, and automated AS9100 compliance documentation"],
        ["AI Document Intelligence", "Automated processing of RFPs, engineering drawings, material certs, ITAR compliance documents, and customer POs \u2014 with full audit trails and role-based access controls"],
        ["Executive AI Briefing", "Half-day leadership workshop: precision manufacturing AI landscape, live demos tailored to Talsco, competitive positioning, and prioritized opportunity assessment"],
    ]
...
story.append(Paragraph("<b>Talsco (JVR Sheetmetal &amp; Fabrication, Inc.)</b>", S_BOLD))
story.append(Paragraph("Attn: Executive Leadership", S_BODY))
story.append(Paragraph("7101 Patterson Dr", S_BODY))
story.append(Paragraph("Garden Grove, CA 92841", S_BODY))
story.append(Spacer(1, 18))
story.append(Paragraph("<b>Dear Talsco Leadership Team,</b>", S_BOLD))
story.append(Spacer(1, 8))

story.append(Paragraph(
    'I\u2019m writing to introduce <font name="CalB" color="#006DB6">Technijian Corporation</font> '
    'and the AI consulting services we provide to established precision manufacturing firms. '
    'With <b>over 55 years in operation, ISO 9001 &amp; AS9100 certification, ITAR registration</b>, '
    'and capabilities spanning water jet cutting, laser cutting, CNC machining, sheet metal fabrication, and assembly, '
    'Talsco represents exactly the type of operationally complex, precision-driven business where '
...
story.append(section_hdr("WHY TALSCO"))
story.append(Spacer(1, 6))
story.append(Paragraph(
    'Your organization has qualities that make AI adoption both high-impact and low-risk: multi-process manufacturing '
    'capabilities serving military, space, and commercial sectors; rigorous quality management under ISO 9001 and AS9100; '
    'ITAR compliance requirements that demand precision in data handling; and a reputation for precision built over more than '
    'five decades of operation.', S_BODY))
story.append(Paragraph(
    'Based on my review of your operations and capabilities, I see several areas where AI can deliver '
    '<b>immediate, measurable ROI</b> for Talsco:', S_BODY))
...
story.append(Spacer(1, 6))
story.append(Paragraph(
    '<font name="CalB" color="#F67D4B">\u25b8 Orange County Neighbor:</font> '
    'We\u2019re based in Irvine, just minutes from your Garden Grove facility \u2014 we understand the SoCal '
    'manufacturing and defense/aerospace supply chain landscape.', S_BODY))
story.append(Paragraph(
...
story.append(Paragraph(
    ...
    'critical when handling ITAR-controlled technical data, AS9100 quality records, and military/space program documents.', S_BODY))
story.append(Paragraph(
    ...
    'and enterprise-grade LLM workflows \u2014 with the same rigor we\u2019d bring to Talsco.', S_BODY))
...
story.append(Paragraph(
    ...
    'Talsco\u2019s operations, production efficiency, and competitive positioning. An '
...
#  BUILD PDF
# ═══════════════════════════════════════════
out_path = os.path.join(os.path.dirname(__file__), "Ravi_Jain_Technijian_Talsco.pdf")
