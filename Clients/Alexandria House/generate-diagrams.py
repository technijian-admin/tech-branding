"""
generate-diagrams.py — Alexandria House (AHO) AI Growth Blueprint
...
Mission-respectful nonprofit framing. AI assists people; it never replaces the relationship.

Output -> Clients/Alexandria House/diagrams/{model,personas,architecture,timeline,competitive}.png
Usage:   py -3.12 "Clients/Alexandria House/generate-diagrams.py"
"""
...
def build_model_svg():
    ...
    title=f'''
  <text x="40" y="34" font-family="Segoe UI,Arial" font-size="18" font-weight="700" fill="{DARK}">How Alexandria House Creates &amp; Sustains Impact</text>
  <text x="40" y="54" font-family="Segoe UI,Arial" font-size="12" fill="{GREY}">Support comes in; the Circle of Support turns it into stability for families; outcomes flow out and, told well, bring more support.</text>'''
    supply=(box(40,92,196,66,WHITE,TEAL,"INDIVIDUAL & MONTHLY DONORS","~83% of revenue (FY2024)",DARK,GREY)
        +box(40,168,196,66,WHITE,TEAL,"FOUNDATIONS & GRANTS","Doheny, Wells, et al.",DARK,GREY)
        +box(40,244,196,66,WHITE,TEAL,"EVENTS & IN-KIND","Walk-A-Thon, wish lists",DARK,GREY))
    sup_label=f'<text x="138" y="82" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{TEAL}" text-anchor="middle">SUPPORT IN</text>'
    hub=box(304,150,196,150,DARK,DARK,"ALEXANDRIA HOUSE","Circle of Support",WHITE,"#C9CDD6",18)
    hub_extra=f'''<text x="402" y="266" font-family="Segoe UI,Arial" font-size="10.5" fill="#C9CDD6" text-anchor="middle">housing · economic opportunity</text>
  <text x="402" y="282" font-family="Segoe UI,Arial" font-size="10.5" fill="#C9CDD6" text-anchor="middle">food · child care · wellness · alumni</text>'''
    demand=(box(568,80,292,56,WHITE,BLUE,"STABLE PERMANENT HOUSING","org reports >92% retention",DARK,GREY)
        +box(568,148,292,56,WHITE,BLUE,"ECONOMIC OPPORTUNITY","Start Up Sisterhood LA",DARK,GREY)
        +box(568,216,292,56,WHITE,BLUE,"CHILDREN THRIVING","child care · afterschool · teen",DARK,GREY)
        +box(568,284,292,56,WHITE,BLUE,"LIFELONG ALUMNI COMMUNITY","support that does not end",DARK,GREY))
    dem_label=f'<text x="714" y="70" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{BLUE}" text-anchor="middle">OUTCOMES FOR FAMILIES</text>'
    arrows=f'''
  ...
  <text x="52" y="374" font-family="Segoe UI,Arial" font-size="11" font-weight="700" fill="{DARK}">PROGRAMS</text>
  <text x="52" y="391" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}">Transitional Housing  ·  Alexandria House Apts (16)  ·  Kenmore (39)  ·  SUSLA  ·  Child Care / Afterschool / Teen  ·  Food · Hygiene · Thrift  ·  Free Therapy / Wellness</text>'''
    band=f'''
  <rect x="40" y="412" width="820" height="62" rx="6" fill="{OFF}" stroke="{LIGHT}"/>
  <rect x="40" y="412" width="6" height="62" rx="3" fill="{ORANGE}"/>
  <text x="60" y="435" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{DARK}">Where Technijian AI plugs in (it assists people; it never replaces the relationship)</text>
  <text x="60" y="456" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}">grant intelligence &amp; drafting  ·  donor development  ·  bilingual EN/ES outreach  ·  impact &amp; funder reporting  ·  volunteer ops  ·  storytelling / AEO</text>'''
...
def build_personas_svg():
    ...
    P=[("MAJOR","Major-Gift Donors",82,82,ORANGE,DARK,30,0,-44,"middle"),
       ("MONTHLY","Monthly / Recurring Donors",50,72,BLUE,WHITE,30,0,-44,"middle"),
       ("FOUND","Foundation Funders",84,54,TEAL,DARK,28,42,2,"start"),
       ("GOVT","Government / Institutional",72,38,GREY,WHITE,24,42,2,"start"),
       ("VOL","Volunteers & Community",30,64,GREEN,WHITE,26,0,-40,"middle"),
       ("CORP","Corporate / In-Kind",48,48,GOLD,DARK,22,0,40,"middle"),
       ("LAPSED","Lapsed Donors",28,28,DARK,WHITE,22,0,40,"middle")]
    dots=""
  ...
  <text x="{X(22)}" y="492" font-family="Segoe UI,Arial" font-size="11.5" font-weight="700" fill="#9AA0A6" text-anchor="middle">RE-ENGAGE</text>'''
    center_note=f'''
  <rect x="{X(40)}" y="{Y(50)-16}" width="160" height="32" rx="6" fill="{WHITE}" stroke="{ORANGE}" stroke-dasharray="4 3"/>
  <text x="{X(40)+80}" y="{Y(50)+4}" font-family="Segoe UI,Arial" font-size="10.5" font-weight="700" fill="{ORANGE}" text-anchor="middle">families served = the purpose</text>'''
    legend=f'''
  <circle cx="84" cy="557" r="7" fill="{ORANGE}"/><text x="96" y="562" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">Major-Gift Donors</text>
  <circle cx="248" cy="557" r="7" fill="{BLUE}"/><text x="260" y="562" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">Monthly / Recurring</text>
  <circle cx="440" cy="557" r="7" fill="{TEAL}"/><text x="452" y="562" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">Foundation Funders</text>
  <circle cx="630" cy="557" r="7" fill="{GREY}"/><text x="642" y="562" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">Government / Institutional</text>
  <circle cx="84" cy="581" r="7" fill="{GREEN}"/><text x="96" y="586" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">Volunteers &amp; Community</text>
  <circle cx="320" cy="581" r="7" fill="{GOLD}"/><text x="332" y="586" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">Corporate / In-Kind</text>
  <circle cx="520" cy="581" r="7" fill="{DARK}"/><text x="532" y="586" font-family="Segoe UI,Arial" font-size="12" fill="{DARK}">Lapsed Donors</text>'''
    return f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 600" width="900" height="600" role="img">
  <text x="80" y="28" font-family="Segoe UI,Arial" font-size="18" font-weight="700" fill="{DARK}">Supporter &amp; Stakeholder Map — Giving Capacity x Engagement</text>
  <text x="80" y="48" font-family="Segoe UI,Arial" font-size="12" fill="{GREY}">Where AI helps Alexandria House steward each supporter relationship. Bubble size approximates revenue weight; calibrate at discovery.</text>
...
def build_arch_svg():
  ...
  <text x="148" y="116" font-family="Segoe UI,Arial" font-size="10" fill="#C9E3F4" text-anchor="middle">visibility · bilingual · AEO</text>
  <rect x="316" y="76" width="264" height="48" fill="{ORANGE}" rx="4"/>
  <text x="448" y="100" font-family="Segoe UI,Arial" font-size="13" font-weight="700" fill="{DARK}" text-anchor="middle">RAISE MORE &amp; WIN GRANTS</text>
  <text x="448" y="116" font-family="Segoe UI,Arial" font-size="10" fill="{DARK}" text-anchor="middle">donors · grants · prospects</text>
  ...
  <text x="748" y="116" font-family="Segoe UI,Arial" font-size="10" fill="{DARK}" text-anchor="middle">families · volunteers · funders</text>'''
    bg=f'''
  ...
  <rect x="616" y="124" width="264" height="412" fill="{OFF}" rx="4"/>'''
    inbound=(arch_card(20,136,256,88,BLUE,"Bilingual Website & Content","Spanish-language donate, help, and story pages for a Latino/Korean neighborhood — the biggest gap.","My SEO",BLUE)
        +arch_card(20,236,256,88,BLUE,"Authority & Impact Stories","A real story engine: outcomes, alumni voices, the 92% — fresh content donors and search reward.","My SEO",BLUE)
        +arch_card(20,336,256,88,BLUE,"AEO / GEO + Google Ad Grant","Be the cited answer for help and giving; activate the $10K/mo free Google Ad Grant.","My SEO",BLUE)
        +arch_card(20,436,256,88,BLUE,"Search & Local Discovery","Own 'transitional housing women children LA' instead of leaking to directories.","My SEO",BLUE))
    core=(arch_card(320,136,256,88,ORANGE,"Grant Prospecting & Drafting","Match funders (Candid/Instrumentl) and draft boilerplate; staff write the funder-specific case.","My AI",TEAL)
        +arch_card(320,236,256,88,ORANGE,"Donor Development & Scoring","Segment, find likely major/recurring givers, re-engage lapsed donors across the ~83% base.","My AI",TEAL)
        +arch_card(320,336,256,88,ORANGE,"Personalized Appeals","Draft tailored, bilingual appeals and thank-yous; a human owns every relationship.","My AI",TEAL)
        +arch_card(320,436,256,88,ORANGE,"Unified Donor View","Reconcile NationBuilder + Givebutter into one 360-degree supporter record.","My Dev",DARK))
    outbound=(arch_card(620,136,256,88,TEAL,"Impact & Funder Reporting","Auto-draft outcomes dashboards and report narratives consistently across many funders.","My AI",TEAL)
        +arch_card(620,236,256,88,TEAL,"Bilingual Comms at Scale","Spanish newsletters, intake, social, surveys — AI drafts, bilingual staff reviews, publish.","My AI",TEAL)
        +arch_card(620,336,256,88,TEAL,"Volunteer & Inquiry Chatbot","Answer routine donor/volunteer questions 24/7; self-serve volunteer & event sign-up.","My Dev",DARK)
        +arch_card(620,436,256,88,TEAL,"Staff Time Recovered","Less admin, more mission: hours returned to families and the relationships that close gifts.","My AI",TEAL))
    arrows=f'''
  ...
  <text x="596" y="324" font-family="Segoe UI,Arial" font-size="10" fill="{GREY}" text-anchor="middle">serve</text>'''
    title=f'''
  <text x="16" y="34" font-family="Segoe UI,Arial" font-size="18" font-weight="700" fill="{DARK}">Alexandria House AI Growth Engine</text>
  <text x="16" y="56" font-family="Segoe UI,Arial" font-size="12" fill="{GREY}">Be found and tell the story (left); raise more and win grants (center); serve families and report impact (right).</text>'''
    note=f'<text x="16" y="552" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}" font-style="italic">* My SEO · My AI · My Dev. AI drafts and researches; a human verifies every claim and owns every donor and family relationship. No client data goes into public AI tools.</text>'
...
def build_timeline_svg():
  ...
  <text x="820" y="{TL_Y+TL_H+18}" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}" text-anchor="middle">Day 365</text>'''
    ...
    cards=(milestone_card(COL_X[0],R1,232,92,BLUE,"1","Bilingual + Ad Grant","Spanish donate/help pages; activate the free Google Ad Grant.","My SEO",BLUE)
        +milestone_card(COL_X[0],R2,232,92,BLUE,"2","AI Readiness + Policy","Map fundraising workflows; AI use policy + light training.","My AI",TEAL)
        +milestone_card(COL_X[1],R1,232,92,TEAL,"3","Grant Engine","Funder matching + first AI-assisted grant drafts.","My AI",TEAL)
        +milestone_card(COL_X[1],R2,232,92,TEAL,"4","Donor Development","Segment donors, score prospects, re-engage lapsed.","My AI",TEAL)
        +milestone_card(COL_X[2],R1,236,92,ORANGE,"5","Unified View + Chatbot","Reconcile the two donor tools; add a volunteer chatbot.","My Dev",DARK)
        +milestone_card(COL_X[2],R2,236,92,ORANGE,"6","Impact Reporting + Dashboard","Automated funder reports + a measured ROI dashboard.","My AI",TEAL))
    note=f'<text x="80" y="378" font-family="Segoe UI,Arial" font-size="11" fill="{GREY}" font-style="italic">* Sequence and targets calibrate at discovery and flex to grant cycles and budget; each phase is sized to pay for itself.</text>'
    title=f'''
  <text x="80" y="28" font-family="Segoe UI,Arial" font-size="18" font-weight="700" fill="{DARK}">90 / 180 / 365-Day Growth Roadmap</text>
  <text x="80" y="48" font-family="Segoe UI,Arial" font-size="12" fill="{GREY}">Alexandria House x Technijian — get found and bilingual, raise more and win grants, deepen and sustain.</text>'''
...
def build_competitive_svg():
    ...
    C=[("ALEXANDRIA HOUSE",58,26,ORANGE,28,0,50,"middle"),
       ("Large LA homeless-services orgs",90,58,BLUE,22,0,-32,"middle"),
       ("Peer family / women's shelters",54,40,TEAL,18,40,2,"start"),
       ("Koreatown / neighborhood orgs",40,30,GREY,16,0,34,"middle"),
       ("Directories (211LA, womenshelters)",78,20,GOLD,18,0,36,"middle"),
    ...
    for label,s,m,color,r,ldx,ldy,anc in C:
        ...
        ring=f'<circle cx="{cx}" cy="{cy}" r="{r+6}" fill="none" stroke="{ORANGE}" stroke-width="2" stroke-dasharray="4 3"/>' if label=="ALEXANDRIA HOUSE" else ""
        dots+=f'''{ring}
  <circle cx="{cx}" cy="{cy}" r="{r}" fill="{color}"/>
  <text x="{lx}" y="{ly}" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{DARK}" text-anchor="{anc}">{esc(label)}</text>'''
    tx,ty=X(70),Y(78); sx,sy=X(58),Y(26)
    move=f'''
  ...
  <text x="{tx+16}" y="{ty-6}" font-family="Segoe UI,Arial" font-size="12" font-weight="700" fill="{ORANGE}" text-anchor="start">The open lane: real mission + AI fundraising</text>'''
    quad=f'''
...
async def main():
    print("Generating Alexandria House diagrams (SVG)...")
