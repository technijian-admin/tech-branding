# Send email signatures to all Admin staff
...
# --- Admin Staff (Admin Assistants + Accountant + CEO) ---
$admins = @(
    @{ Name = "Shaundra Reahle"; Email = "Sreahle@technijian.com" }
    @{ Name = "Alex Alcantar";   Email = "aalcantar@technijian.com" }
    @{ Name = "Navjit Kaur";     Email = "nkaur@technijian.com" }
    @{ Name = "Shelja Mehta";    Email = "smehta@technijian.com" }
)

Write-Host "Sending email signatures to $($admins.Count) Admin staff...`n"

foreach ($adm in $admins) {
    $firstName = ($adm.Name -split " ")[0]
    $sigFile = Join-Path $sigDir "$($adm.Name).html"

    if (-not (Test-Path $sigFile)) {
        Write-Host "  WARNING: No signature file for $($adm.Name) - SKIPPED" -ForegroundColor Yellow
        continue
    }
...
<ol>
<li>Save the attached <b>$($adm.Name).html</b> file to your computer</li>
...
<ol>
<li>Save the attached <b>$($adm.Name).html</b> file and open it in your browser</li>
...
"@
    ...
    $params = @{
        Message = @{
            ...
            ToRecipients = @(
                @{ EmailAddress = @{ Address = $adm.Email } }
            )
            Attachments = @(
                @{
    ...
    try {
        ...
        Write-Host "  Sent to $($adm.Name) ($($adm.Email))" -ForegroundColor Green
    }
    catch {
        Write-Host "  FAILED: $($adm.Name) - $($_.Exception.Message)" -ForegroundColor Red
    }

    Start-Sleep -Seconds 1
}

Write-Host "`nDone! All Admin staff emails sent."
