# Send email signatures to Developer team (Client Application Engineers)
...
# --- Developers (Client Application Engineers) ---
$devTeam = @(
    @{ Name = "Simranjit Singh";     Email = "ssingh@technijian.com" }
    @{ Name = "Tharunaa Shoban Babu"; Email = "tbabu@technijian.com" }
)

Write-Host "Sending email signatures to $($devTeam.Count) developers...`n"

foreach ($dev in $devTeam) {
    $firstName = ($dev.Name -split " ")[0]
    $sigFile = Join-Path $sigDir "$($dev.Name).html"

    if (-not (Test-Path $sigFile)) {
        Write-Host "  WARNING: No signature file for $($dev.Name) - SKIPPED" -ForegroundColor Yellow
        continue
    }
...
<ol>
<li>Save the attached <b>$($dev.Name).html</b> file to your computer</li>
...
<ol>
<li>Save the attached <b>$($dev.Name).html</b> file and open it in your browser</li>
...
<p><b>Important:</b> Your signature includes two booking buttons &mdash; a blue <b>&quot;Book a Meeting&quot;</b> button (your personal booking link) and an orange <b>&quot;Book Dev Meeting&quot;</b> button (developer team booking page). Both are already configured with the correct links.</p>
...
"@
    ...
    $params = @{
        Message = @{
            ...
            ToRecipients = @(
                @{ EmailAddress = @{ Address = $dev.Email } }
            )
            Attachments = @(
                @{
    ...
    try {
        ...
        Write-Host "  Sent to $($dev.Name) ($($dev.Email))" -ForegroundColor Green
    }
    catch {
        Write-Host "  FAILED: $($dev.Name) - $($_.Exception.Message)" -ForegroundColor Red
    }

    Start-Sleep -Seconds 1
}

Write-Host "`nDone! All developer emails sent."
