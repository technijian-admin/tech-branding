# Send email signatures to all Client Support Engineers
...
$engineers = @(
    @{ Name = "Aditya Saraf";          Email = "asaraf@technijian.com" }
    @{ Name = "Ajay Bhardwaj";         Email = "ABhardwaj@technijian.com" }
    @{ Name = "Deepak Bhardwaj";       Email = "Dbhardwaj@technijian.com" }
    @{ Name = "Gurdeep Kumar";         Email = "gkumar@technijian.com" }
    @{ Name = "Parveen Biswal";        Email = "pbiswal@technijian.com" }
    @{ Name = "Rahul Uniyal";          Email = "runiyal@technijian.com" }
    @{ Name = "Rajat Kumar";           Email = "Rkumar@technijian.com" }
    @{ Name = "Rishad Samad";          Email = "Rmohamed@technijian.com" }
    @{ Name = "Revanth Sai Peddestti"; Email = "srevanth@technijian.com" }
    @{ Name = "Sanjeev Kumar";         Email = "skumar@technijian.com" }
    @{ Name = "Satish Sharma";         Email = "ssharma@technijian.com" }
    @{ Name = "Suresh Kumar Sharma";   Email = "Sksharma@technijian.com" }
    @{ Name = "Surinder Kumar";        Email = "sverma@technijian.com" }
    @{ Name = "Yogesh Kumar";          Email = "ykumar@technijian.com" }
)

Write-Host "Sending email signatures to $($engineers.Count) Client Support Engineers...`n"

foreach ($eng in $engineers) {
    $firstName = ($eng.Name -split " ")[0]
    $sigFile = Join-Path $sigDir "$($eng.Name).html"

    if (-not (Test-Path $sigFile)) {
        Write-Host "  WARNING: No signature file for $($eng.Name) - SKIPPED" -ForegroundColor Yellow
        continue
    }

    # Read signature file and base64 encode for attachment
...
<ol>
<li>Save the attached <b>$($eng.Name).html</b> file to your computer</li>
...
<ol>
<li>Save the attached <b>$($eng.Name).html</b> file and open it in your browser</li>
...
<p><b>Important:</b> Your signature includes two booking buttons &mdash; a blue <b>&quot;Book a Meeting&quot;</b> button (your personal booking link) and an orange <b>&quot;Book time with Support&quot;</b> button (team booking page). Both are already configured with the correct links.</p>
...
"@
    ...
    $params = @{
        Message = @{
            ...
            ToRecipients = @(
                @{ EmailAddress = @{ Address = $eng.Email } }
            )
            Attachments = @(
                @{
    ...
    try {
        ...
        Write-Host "  Sent to $($eng.Name) ($($eng.Email))" -ForegroundColor Green
    }
    catch {
        Write-Host "  FAILED: $($eng.Name) - $($_.Exception.Message)" -ForegroundColor Red
    }

    Start-Sleep -Seconds 1
}

Write-Host "`nDone! All Client Support Engineer emails sent."
