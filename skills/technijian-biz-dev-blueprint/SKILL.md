...
> **Nonprofit clients (AHO, 2026-06-16) — adapt three things.** (1) *"Growth" = funding + mission capacity, not sales.* The three channels become: be found & tell the story (bilingual website, impact stories, AEO, **Google Ad Grants** = $10K/mo free search ads); raise more & win grants (grant prospecting/drafting, donor segmentation & major-gift prospect research, lapsed-donor re-engagement); serve & report impact (funder reporting automation, multilingual comms, volunteer chatbot, staff-time recovery). (2) *No ROI multiples.* A "1.7×/4.3×" framing is tone-deaf for a deficit-year mission — frame return as **dollars raised + staff hours recovered + people served**, modeled after discovery. (3) *Affordability is a feature.* Lead with the free/granted 501(c)(3) stack (Google Ad Grants, free Google Workspace, M365 nonprofit + Copilot via TechSoup, Candid Foundation Directory free via Gold Seal) so the plan reads as right-sized. Add nonprofit guardrails to Section 09: never feed beneficiary PII to public AI; verify every funder claim (one fabricated stat ends a grant relationship); follow each funder's AI-disclosure rule (NIH/NSF/Spencer/Wellcome); a bilingual native-reviewer quality gate.

> **Post-decline reframe — the cold lead who said no to one service.** When the prospect replied to outreach by declining a specific service ("we already have our network team in place"), do NOT re-pitch that service. Open the deliverable (page-2 callout) by acknowledging the decline respectfully ("keep your network team") and pivot the entire blueprint to adjacent value the incumbent does NOT provide — for AHO, managed IT was off the table, so the whole plan became AI for mission growth. A reply, even a "no," is an opening; the decline tells you exactly which lane is closed so you can own an open one. (See `feedback_reframe_off_declined_service` in auto-memory.)
...
**Manual (non-field) TOC caveat — AHO, 2026-06-16:** some build-script lineages (the CHR/AHO family) hand-build the Contents page as a list of `Paragraph`s instead of a Word `TableOfContents` field. That is fine ONLY if the manual list (a) is placed right after the cover, and (b) carries a real page number on every row. The AHO bug was a manual TOC that listed 15 section titles with NO page numbers and sat AFTER the "How to Read" note — so it neither reflected the 35-page document nor sat where a reader looks first. Fix pattern: a `tocRow(num, title, page)` helper with two tab stops — `{type: LEFT, position: 600}` for the title and `{type: RIGHT, position: CONTENT_W, leader: LeaderType.DOT}` for the page number (import `LeaderType`) — one row per front-matter item AND section; then move the `toc` block to right after `cover` in the children array; then build → render → search the PDF for each heading's true start page → write those numbers in → rebuild. (A manual TOC's numbers are static — recompute and re-verify after any content edit. If you'd rather not maintain numbers by hand, switch that script to the field-based `TableOfContents` below, which Word repopulates on update.)
...
**The canonical PDF gets LOCKED when the client (or you) has it open in a viewer (AHO, 2026-06-16).** The overwrite then fails with `PermissionError: [WinError 32] ... being used by another process`, and — worse — a silent variant where the DOCX rebuilds fine but the PDF on disk stays the OLD version, so you "fix" something and the viewer still shows the defect. Two rules: (1) if you're iterating while the user is reviewing, render your verification PDF to a **temp name** (`_verify/<code>-check.pdf`) so you can confirm the fix without fighting the lock; (2) before finalizing the canonical-named PDF, make sure the viewer is closed (the WinError 32 IS the tell that it's open) — ask the user to close it, then write the canonical name and re-attach to any email draft. After ANY blueprint edit that the client has already received, the email attachment is stale until you re-run the draft script against the freshly-written PDF.
...
- **Scan the BUILD-SCRIPT source for banned words, not just the rendered text** — and use a RELIABLE grepper. (AHO, 2026-06-16: a Bash `grep` for banned words returned "none found," but an `rtk`/`rg` wrapper on PATH had silently hijacked the command and broken it — three banned words shipped past it: `leverage` ×2 and `vendor-neutral`. Re-running with the dedicated **Grep tool** (ripgrep, permission-integrated) surfaced all three.) Rule: run the banned-word scan with the Grep tool; if you must use Bash `grep`, prove it actually ran (grep a word you KNOW is present and confirm a hit) before trusting a "clean" result. Watch the compounds too: `highest-leverage`, `vendor-neutral`, `vendor-agnostic` all contain banned roots — replace with `highest-impact`, `impartial`, `independent`.

### Gate 3: Visual / PDF audit
- [ ] **Contents page placement + page numbers (AHO, 2026-06-16):** the Contents page must sit **immediately after the cover** — BEFORE any "How to Read This Blueprint" / method note / other front matter (a Contents page buried after the method note reads as "wrong place"). The TOC must show a **right-aligned page number (dot leader) for EVERY entry** — front matter included (How to Read, Executive Summary) plus all numbered sections, About, and Appendix. A TOC that lists section titles with NO page numbers is a defect — it doesn't reflect the real N-page document.
- [ ] **Verify every TOC page number against the rendered PDF.** For each entry, search the PDF for the section heading and confirm its ACTUAL first page equals the TOC number. Gotcha: the Contents page itself contains every title, so a naive search matches page 2 for everything — exclude the TOC page and match on unique section-BODY text, not the title. (Manual/field both: re-verify after ANY content change, since lengths shift pages.)
- [ ] **Section numbers must not be mistakable for page coverage (AHO, 2026-06-16 — burned THREE times on this).** If the TOC has a left **section-number** column (01, 02 … 15), a reader reads the highest number as "the report stops at 15" and complains the TOC "doesn't cover the 35 pages" — even when the right-hand page numbers are correct. The page numbers alone don't fix the perception; the bold section numbers dominate. Mitigate with ALL THREE: (a) a one-line clarifier under "Contents" — e.g. *"Fifteen sections across 35 pages. The orange figures are section numbers; the figures on the right are the pages each section spans."*; (b) labeled columns — `#` / `SECTION` / `PAGES`; (c) **page RANGES not single start-pages** (`7–8`, `9–10`, … `34–35`) so the page column visibly runs contiguous from the first body page to the LAST page of the report. The last TOC row reaching `34–35` is the proof it covers the whole document. Compute ranges as `start-of-this … (start-of-next − 1)`; verify against the render.
